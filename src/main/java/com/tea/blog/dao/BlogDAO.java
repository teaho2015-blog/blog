...
public interface BlogDAO {
    ...
    @Select("SELECT * FROM B_BLOG ORDER BY CREATE_TIME DESC LIMIT #{0},#{1}")
    public List<Blog> getBlogList(long preIndex, int range);



    @Select("SELECT * FROM B_BLOG A WHERE ID=#{0} ")
 ...
 */
    @Select("SELECT * FROM B_BLOG A WHERE A.CREATE_TIME < (SELECT CREATE_TIME FROM B_BLOG WHERE ID=#{0})  ORDER BY A.CREATE_TIME DESC LIMIT 1 ")
    public Blog getPreOne(String id);

    @Select("SELECT A.*, B.ID AS ELDERID FROM B_BLOG A left join (SELECT T.*,#{0} AS TEMPID  FROM B_BLOG T WHERE T.CREATE_TIME < (SELECT CREATE_TIME FROM B_BLOG WHERE ID=#{0}) ORDER BY T.CREATE_TIME DESC LIMIT 1) B ON B.TEMPID=A.ID WHERE A.ID=#{0} ")
