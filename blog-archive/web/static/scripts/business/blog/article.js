...
ArticleAnimator.getPost = function(index, callback){
  ...
  if ( this.postCache[index] ){
    callback( this.postCache[index] ,index );
    return;
  }

  var self = this;
  //use it in controller to create article content
  $.getJSON(getRootPath()+'/blog/article/rand/'+ index, function(d){
...
ArticleAnimator.createPost = function(opts, callback){
  ...
  this.getPost(index, function(d, index){
    self.contentizeElement(self['$' + type], d, index);
    callback && callback();
  });

}

ArticleAnimator.contentizeElement = function($el, d, index){
  $el.find('.big-image').css({ backgroundImage: "url(" + getRootPath() + "/" + d.blog.image_url + ")" });
  $el.find('h1.title').html(d.blog.title);
  $el.find('h2.description').html(d.blog.title_secondary);
  $el.find('.content .text').html(d.blog.content);
  $el.find('h3.byline time').html(d.blog.date);
  $el.find('h3.byline .author').html('<a href=\"' + getRootPath() + '/blog/'+ d.blog.creator_id +'\" >'+d.blog.creator_name+'</a>');
...
ArticleAnimator.setCurrentLikeStatus = function($el,d,index) {
    var likeStas = $el.find('.post-footer .like');
    //tooltip can use tooltip plugins....
//    likeStas.addClass("tooltip-test");
//    likeStas.attr("data-original-title","like this");
    if(d.like=="0"){
        likeStas.addClass("liked");
    }else {
        likeStas.removeClass("liked");
    }
    this.activatePostComponents($el,index);

}

ArticleAnimator.setCurrentTooltip= function() {
    //bootstrap tooltop plugin
...
ArticleAnimator.activatePostComponents = function($el,index) {
    var article = this;
    $el.find(".content .like").bind("click",function(){
        var self = $(this);
        var stat = self.attr("class").indexOf("liked")>=0;
        var blogId = self.prev();
        if(stat){
            $.ajax({
                url: getRootPath()+'/blog/'+blogId.val()+'/like',
                type:'DELETE',
                success:function(result) {
                    self.removeClass("liked");
                    article.postCache[index].like = '1';
//                    console.log(index);
//                    console.log(article.postCache[index]);
//                    console.log(article.postCache[index].like);
                },
                error:function(result) {
                    alert("Sorry,we seem to be having technical trouble.\nPlease try again later!");
                }
            ...
            $.ajax({
                url: getRootPath()+'/blog/'+blogId.val()+'/like',
                type:'POST',
//                    data:{title:$title.val(),title_secondary:$titleSecondary.val(),content:$content.val()},
                success:function(result) {
                    self.addClass("liked");
                    article.postCache[index].like = '0';
                },
                error:function(result) {
                    alert("Sorry,we seem to be having technical trouble.\nPlease try again later!");
                }
