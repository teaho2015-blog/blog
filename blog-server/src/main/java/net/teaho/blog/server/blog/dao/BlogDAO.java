 ...
 */
    ...
    @Select("SELECT A.*, B.ID AS ELDERID FROM B_BLOG A LEFT OUTER JOIN (SELECT T.*,#{arg0} AS TEMPID  FROM B_BLOG T WHERE T.CREATE_TIME < (SELECT CREATE_TIME FROM B_BLOG WHERE ID=#{arg0}) ORDER BY T.CREATE_TIME DESC LIMIT 1) B ON B.TEMPID=A.ID WHERE A.ID=#{arg0} AND A.DELETE_FLAG=0 ")
